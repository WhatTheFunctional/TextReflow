...
splitString delimiters (c : cs) [] allWords
    | c `elem` delimiters = splitString delimiters cs [] allWords --Splitting character detected and no current word exists
    | otherwise = splitString delimiters cs [c] allWords --A non-splitting character was detected and no current word exists
splitString delimiters (c : cs) currentWord allWords
    | c `elem` delimiters = (reverse currentWord) : splitString delimiters cs [] allWords --Splitting character detected and a current word exists
    | otherwise = splitString delimiters cs (c : currentWord) allWords --A non-splitting character was detected
...
splitStringWithDelimiters delimiters (c : cs) [] allWords --Current word is empty
    = splitStringWithDelimiters delimiters cs [c] allWords --Create new word with the current character
splitStringWithDelimiters delimiters (c : cs) currentWord@(cw : cws) allWords
    | (not (c `elem` delimiters)) && (cw `elem` delimiters) ||
      (c `elem` delimiters) && (not (cw `elem` delimiters)) = (reverse currentWord) : splitStringWithDelimiters delimiters (c : cs) [] allWords --New split detected and a current word exists
    | otherwise = splitStringWithDelimiters delimiters cs (c : currentWord) allWords --A non-splitting character was detected, add it to the current word
